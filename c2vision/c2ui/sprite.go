...
type Sprite struct {
	*BaseWidget
	animation c2interface.Animation
	*c2util.Logger
}
...
func (s *Sprite) Render(target c2interface.Surface) {
...
func (s *Sprite) GetSurface() c2interface.Surface {
	return s.animation.GetCurrentFrameSurface()
}

// RenderSection renders the section of the sprite enclosed by bounds
func (s *Sprite) RenderSection(sfc c2interface.Surface, bound image.Rectangle) {
...
func (s *Sprite) RenderSegmented(target c2interface.Surface, segmentsX, segmentsY, frameOffset int) {
	var currentY int
	...
	for y := 0; y < segmentsY; y++ {
		...
		for x := 0; x < segmentsX; x++ {
			...
			maxFrameHeight = c2math.MaxInt(maxFrameHeight, frameHeight)
			currentX += frameWidth
		}
...
func (s *Sprite) SetEffect(e c2enum.DrawEffect) {
