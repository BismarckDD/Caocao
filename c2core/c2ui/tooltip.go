...
// static check that Tooltip implements Widget interface.
...
func (t *Tooltip) Render(target c2interface.Surface) {
	maxW, maxH := t.GetSize()
	...
	// tooltip background
	if t.boxEnabled {
		target.DrawRect(maxW, maxH, c2util.Color(blackAlpha70))
	}
