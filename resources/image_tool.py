...
def resize(input_path, old_path, new_path, scale):

    # file_name = input_path.rpartition("/")[2]
    # print(input_path.rpartition("/"))

    output_file = input_path.replace(old_path, new_path)
    with Image.open(input_path) as image:
        # image.show()
...
if __name__ == '__main__':
    ...
    if option == "resize":
        ...
        img_file_list = find_files(input_path)
        # print("len: ", str(len(img_file_path_list)))
        for img_file_path in img_file_list:
            if os.path.exists(img_file_path):
                resize(img_file_path, "normal_res/item", "high_res/item2", scale)
        print("Analyze...Done.")
    elif option == "rename":
